...
private:
	...
	void			get_data( std::vector<Channel> &ch );
	static int		get_number_of_bytes( std::string infile );
	static bool		positioning( std::ifstream &tbl, char* label);
	void			read_TSn_first_tag( ifstream &infile, StationFile *ts, size_t position );
	void			read_TSn_tag( ifstream &infile, StationFile *ts, size_t position );
	void 			read_TSn_time_series( std::ifstream &infile, std::vector<Channel> &ch, size_t counter );
